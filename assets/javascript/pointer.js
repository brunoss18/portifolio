...
$(document).ready(function() {
  ...
  $(document).keypress(function(event) {
    ...
    if (event.key == "") spread = 1;
    if (event.key == "") spread = 2;

    if (event.key == "") pathMode = 1;
    if (event.key == "") pathMode = 2;
...
function animatePoints() {
  ...
  for (var i = 0; i < points.length; i++) {
    ...
    context.strokeStyle = 'rgb(' + Math.floor(8) + ',' +
      Math.floor(252 + (1 * dec)) + ',' +
      Math.floor(215 + (1 * inc)) + ')';
